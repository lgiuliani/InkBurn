...
class GCodeGenerator:
    ...
    def add_header(self, document_name: str = "") -> None:
        ...
        self.add_comment("Generated by Inkscape InkBurn extension")
        self.add_comment(f"Document: {document_name}")
        self.add_comment(f"Date: {now}")
    ...
    def add_job(
        ...
        self.add_comment(f"Job: {job.type.value} {job_index} (id={job.id})")

        for pass_num in range(passes):
            if passes > 1:
                self.add_comment(f"--- Pass {pass_num + 1}/{passes}")
