...
def _linear_gradient_point(
    ...
    if units == "userSpaceOnUse":
        ...
        x2 = _parse_length(_gradient_attr(chain, "x2", "100%"), width, min_x + width)
...
def _sample_radial_gradient(
    ...
    if units == "userSpaceOnUse":
        cx = _parse_length(_gradient_attr(chain, "cx", "50%"), width, min_x + width / 2)
        cy = _parse_length(
...
def _apply_fill_power_to_hatch(
    hatch: PathSegment,
    ...
    if _fill_uses_gradient(shape):
        ...
        hatch.power = None
        hatch.powers = [
            _fill_power(shape, job, point, viewbox_height, bbox)
            for point in hatch.points
        ]
        return

    hatch.power = _fill_power(shape, job)
    hatch.powers = None
...
class ExportGCode(inkex.OutputExtension):
    ...
    def _process_fill_job(
        ...
        for shape, segments in segment_groups:
            ...
            bbox = _polygons_svg_bbox(closed_polygons, viewbox_height)
